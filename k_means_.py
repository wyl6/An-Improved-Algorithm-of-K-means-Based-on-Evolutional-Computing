...
def _kmeans_single_lloyd(X, sample_weight, n_clusters, max_iter=300,
    ...
    ######################################################################
    # simulated annealing addition
    new_distances = distances.copy()
    swim_distances = distances.copy()
    final_distances = distances.copy()
    
    ######################################################################
    # random swimming addition
    max_col = np.tile(np.max(X, axis=0), (c_row, 1))
    min_col = np.tile(np.min(X, axis=0), (c_row, 1))
    del_col = max_col-min_col
#    print('max_col', max_col)
#    print('min_col', min_col)
#    print('del_col', del_col)
    ######################################################################
    b_centers = np.random.random((c_row, c_col))
    b_inertia = float('inf')
    b_labels = None
    ######################################################################
    # initial temperature
...
#                final_centers = centers_old+(new_centers-centers_old)*0.2
...
#                
#            else:
#                final_centers = centers_old+(centers_old-new_centers)*0.2
##                final_centers = centers_old+(new_centers-centers_old)*0.3
#                final_labels, final_inertia = _labels_inertia(X,  sample_weight, x_squared_norms, final_centers,
#                                precompute_distances=precompute_distances,distances=final_distances)
#                if(final_inertia<b_inertia):
#                    b_inertia = final_inertia
...
##            new_centers = centers_old+(new_centers-centers_old) * np.random.random((c_row, c_col)) # not very good
#            new_centers = centers_old+(new_centers-centers_old) * 0.5 # stable and good
#            new_labels, new_inertia = _labels_inertia(X, sample_weight, x_squared_norms, new_centers,
#                            precompute_distances=precompute_distances,distances=new_distances)
#            if(new_inertia<b_inertia):
#                b_inertia = new_inertia
...
#                    centers_old = final_centers.copy()
        ...
        ######################################################################
        # todo: atrificial fish stocks, not very good
        ######################################################################
#        flight = levy_flight(Lambda=1.5, dim=[c_row, c_col])
#        flight = normalize(flight, norm='max', axis=0)
#        flight = normalize(flight.reshape([1, c_col]), norm='l1')
#        flight = flight.reshape(c_col)
#        swim_centers = min_col+del_col *flight
#        swim_centers = min_col+del_col * np.random.random((c_row, c_col))
#
#        
#        forge_number = 10
#        for i in range(forge_number):
#            
#            new_centers = min_col+del_col * np.random.random((c_row, c_col))
#        
#            flight = levy_flight(Lambda=1.5, dim=[c_row, c_col])
#            flight = normalize(flight, norm='max', axis=0)
#            new_centers = swim_centers+del_col * flight * 0.3
##            new_centers = swim_centers+(new_centers-swim_centers) * 0.5
#            
#            
...
#            
#            if(new_inertia < inertia):
##                flight = levy_flight(Lambda=1.5, dim=c_col)
##                flight = normalize(flight.reshape([1, c_col]))
##                flight = flight.reshape(c_col)
##                final_centers = centers_old+(new_centers-centers_old)*flight
#                final_centers = centers_old+(new_centers-centers_old)*0.3
#                final_labels, final_inertia = _labels_inertia(X, x_squared_norms, final_centers,
#                                precompute_distances=precompute_distances,
#                                distances=final_distances)
#                labels = final_labels.copy()
#                inertia = final_inertia.copy()
...
#                centers_old = final_centers.copy()
#            else:
#                final_centers = centers_old+(centers_old-new_centers)*0.05
#                final_labels, final_inertia = _labels_inertia(X, x_squared_norms, final_centers,
#                                precompute_distances=precompute_distances,
#                                distances=final_distances)
#                labels = final_labels.copy()
    ...
    if center_shift_total > 0:
        ...
        best_labels, best_inertia = \
            _labels_inertia(X, sample_weight, x_squared_norms, best_centers,
                            precompute_distances=precompute_distances,
                            distances=distances)
    if(best_inertia>b_inertia):
        best_centers = b_centers
        best_inertia = b_inertia
        best_labels = b_labels
...
#
    def get_prediction(self, label_i):
        y_i = self.y[self.labels_ == label_i]
        return np.argmax(np.bincount(y_i))
    ...
    def get_labels(self, X):
        ...
        for i in range(self.n_clusters):
            predictions[self.labels_ == i] = np.argmax(np.bincount(self.y[self.labels_ == i]))
        return predictions
